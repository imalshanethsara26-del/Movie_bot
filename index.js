                ...
                if (/^\d+$/.test(cmd) && userSessions[from]) {
                    ...
                    if (session.type === 'movie_search') {
                        ...
                        try {
                            ...
                            const rYear = rData.year || rData.date || 'N/A';
                            const imdbR = rData.rating || rData.imdb || 'N/A';
                            const genre = parseGenre(rData.category || rData.genre || rData.genres);
                            const rTime = rData.runtime || rData.duration || 'N/A';
                            const cast = parseCast(rData.cast || rData.actors || rData.actor);
                            const plot = (rData.description || rData.plot || rData.overview || rData.story || rData.sinopsis || 'තොරතුරු නොමැත.').replace(/<[^>]*>?/gm, '').trim();
                            const posterUrl = rData.image || rData.img || rData.poster || item.image || item.poster;

                            const dList = rData.downloads || rData.result?.downloads || rData.data?.downloads || (Array.isArray(rData) ? rData : []);
                            const vDownloads = dList.filter(i => {
                                const l = (i.link || i.url || '').toLowerCase();
                                const q = (i.quality || i.title || '').toLowerCase();
